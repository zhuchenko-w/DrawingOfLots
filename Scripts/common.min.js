$(function(){createNumberOfGroupsOptions();$(".values").tooltip({trigger:"manual",placement:"right"});$("#all-countries").on('change',function(){if($(this).is(":checked")){showValues(AllCountries)}});$("#all-states").on('change',function(){if($(this).is(":checked")){showValues(AllStates)}});$("#other").on('change',function(){if($(this).is(":checked")){showValues(null)}});$(".values").on("change input",function(){var text=$(this).val();if(text==""){$(".lines-count").text("")}else{var lines=text.split(/\r|\r\n|\n/).filter(function(s){return s!==''});$(".lines-count").text("("+lines.length+") ")}});$(".start-btn").click(draw);showValues(AllCountries)});function createNumberOfGroupsOptions(){var i=1;while(i++<11){$(".number-of-groups").append($("<option value='"+i+"'"+(i==4?"selected":"")+">"+i+" штук"+(i>4?"":"и")+"</option>"))}}
function showValues(values){var textArea=$(".values");if(values==null){textArea.val("").change();textArea.tooltip("show")}else{textArea.val(values.join("\n")).change();textArea.tooltip("hide")}}
function draw(){var values=$(".values").val().split("\n").filter(function(s){return s!==''});var groupSize=parseInt($(".number-of-groups").val());var resultsDiv=$(".results");shuffle(values);var groups=split(values,groupSize);resultsDiv.html("");var i,j;for(i=0;i<groups.length;i++){var html="<div class='result-group col'><ul class='list-group'>";for(j=0;j<groups[i].length;j++){html+="<li class='list-group-item'>"+groups[i][j]+"</li>"}
...
if(result.length>1&&result[result.length-1].length<groupSize){result[result.length-2]=result[result.length-2].concat(result[result.length-1]);result.pop()}
return result}
